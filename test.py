...
parser.add_argument('--name', type=str, default='/data/ORGAN')
parser.add_argument('--gpu', type=str, default='5')
parser.add_argument('--save_per_class', action='store_true', default=True, help='Save individual images per class in addition to grid images')
args = parser.parse_args()

conf_file = os.path.join(args.name, 'colon.yaml')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for it, (imgs, labels) in tqdm(enumerate(test_dataloader)):
            imgs = imgs.cuda()
            fake_xs = []
            ...
            write_image(it, out_dir, imgs, fake_xs, format='png',
                        save_per_class=args.save_per_class, class_labels=labels)
